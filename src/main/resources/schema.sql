...
create table actividad(a_id int primary key not null, ta_nombre VARCHAR(20) not null, a_dia DATE not null, a_ini int not null, a_fin int not null, FOREIGN KEY (ta_nombre) REFERENCES TIPOACTIVIDAD (ta_nombre), CHECK (a_id > 0), CHECK (8 <= a_ini <= 22), CHECK (9 <= a_fin <= 22));
create table actividadlimitada(a_id int primary key not null, a_plazas int not null, FOREIGN KEY (a_id) REFERENCES ACTIVIDAD (a_id), CHECK (a_plazas > 0));
